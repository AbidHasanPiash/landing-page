...
export default function CareerPage() {
    ...
    const filteredPosts = posts.filter((post) => {
        ...
        return (
            ...
            post.experience.toLowerCase().includes(searchLower)
        );
    });
        ...
        <section>
                                ...
                                <div key={index} className="flex justify-between space-x-2 border rounded-xl p-5">
                                    ...
                                    <div className="w-full space-y-3">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            {post.tags.map((tag, i) => (
                                                <div key={i} className="w-fit px-2 py-1 rounded-md bg-secondary/40">
                                                    {tag}
                                                </div>
                                            ))}
                                        </div>
                                        <div className="flex items-center divide-x divide-gray-300">
                                            <p className="flex flex-col md:flex-row md:items-center space-x-1 pr-2">
                                                <span>
                                                    <HiOutlineLocationMarker />
                                                </span>
                                                <span>{post.location}</span>
                                            </p>
                                            <p className="flex flex-col md:flex-row md:items-center space-x-1 px-2">
                                                <span>
                                                    <FiGitBranch />
                                                </span>
            ...
            <div className="max-w-8xl mx-auto w-full h-full px-4 md:px-8 lg:px-20 space-y-20 mb-20">
                <div className="rounded-3xl bg-muted-hover px-10 pt-10 lg:px-20 lg:pt-20 space-y-10 flex flex-col items-center justify-center">
                    <h1 className="text-2xl lg:text-3xl xl:text-4xl text-center font-semibold">
